...
export function CreateBatchDialog({ shops, hubs, onClose }: { shops: Shop[]; hubs: Hub[]; onClose: () => void }) {
  ...
  const { user } = useAuth();
  const canPay = ["SUPERADMIN", "FINANCE"].includes(user?.role ?? "");
  ...
  const restored = (() => { try { return recovered ? JSON.parse(recovered) as { shopId: string; hubId?: string; pickupDate: string; batchName: string; wallets: { cash: number; kbzPay: number; wavePay: number } } : null; } catch { return null; } })();
  ...
  const total = watch(["cash", "kbzPay", "wavePay"]).reduce((sum, value) => sum + (Number.isFinite(value) ? value : 0), 0);
  const create = useMutation({
    mutationFn: (values: Values) => { request.current ??= JSON.stringify({ shopId: values.shopId, pickupDate: values.pickupDate, batchName: values.batchName, advancePaid: canPay ? total : 0, wallets: { cash: canPay ? values.cash : 0, kbzPay: canPay ? values.kbzPay : 0, wavePay: canPay ? values.wavePay : 0 }, idempotencyKey: idempotencyKey.current, ...(values.hubId ? { hubId: values.hubId } : {}) }); persistRequest(request.current); return api<CreatedBatch>("/operations/batches", { method: "POST", body: request.current }); },
  ...
  return <div className="fixed inset-0 z-30 grid place-items-center overflow-y-auto bg-black/50 p-4"><div role="dialog" aria-modal="true" aria-labelledby="batch-title" className="w-full max-w-2xl rounded-3xl bg-white p-6 text-slate-950 shadow-2xl dark:bg-[#181a1d] dark:text-white">
    ...
    <form onSubmit={handleSubmit(values => { if (sending.current) return; sending.current = true; create.mutate(values); })} className="mt-6 grid gap-4 md:grid-cols-2">
      ...
      {canPay && <div className="rounded-xl bg-slate-50 p-4 dark:bg-white/5 md:col-span-2"><h3 className="font-bold">{t("totalAdvancePaid")}</h3><p className="mt-1 text-sm text-slate-500">{t("advanceAutomaticHelp")}</p><div className="mt-3 grid gap-3 sm:grid-cols-3">{(["cash", "kbzPay", "wavePay"] as const).map(wallet => <label key={wallet} className="text-sm font-bold">{t(wallet)}<input aria-label={t(wallet)} type="number" min="0" step="1" {...register(wallet, { valueAsNumber: true, min: 0, validate: value => Number.isSafeInteger(value) })} className={`${control} mt-2 w-full`} /></label>)}</div><p className="mt-3 font-semibold">{t("totalAdvancePaid")}: {total.toLocaleString()} {t("mmk")}</p></div>}
      </fieldset>
      ...
      <div className="flex justify-end gap-3 md:col-span-2"><button type="button" disabled={locked || create.isPending} onClick={onClose} className={control}>{t("cancel")}</button><button type={locked ? "button" : "submit"} onClick={locked ? () => { if (!sending.current) { sending.current = true; create.mutate(watch()); } } : undefined} disabled={create.isPending} className="rounded-xl bg-[#1598ef] px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{create.isPending ? t("loading") : locked ? t("retry") : t("saveAndAddParcels")}</button></div>
    </form>
  </div></div>;
}
