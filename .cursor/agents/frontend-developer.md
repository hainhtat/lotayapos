...
- If PROJECT_SPEC.md conflicts with explicit user requirements, follow the user and call out the conflict. If implementation reveals an intentional architectural or behavioral deviation, update PROJECT_SPEC.md only when requested or when the task includes documentation synchronization; otherwise report the deviation for the Spec Maintainer.
