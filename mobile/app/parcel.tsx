...
export default function Parcel(){
  ...
  const call=async()=>{const result=await callCustomer(parcelQuery.data?.customerPhone,Linking);if(result!=="opened")Alert.alert(i18n.t("callUnavailableTitle"),i18n.t(result==="missing"?"phoneMissing":"callUnavailable"))};
  ...
  return <SafeAreaView style={[s.safe,dark&&s.dark]}><ScrollView contentContainerStyle={s.content}>
    ...
    <View style={s.options}>{(["DELIVERED","PARTIAL","FAILED","REJECTED"] as Outcome[]).map(value=><Pressable accessibilityRole="radio" accessibilityState={{selected:choice===value}} key={value} onPress={()=>{setChoice(value);setReason(null);setNote("");setError("")}} style={[s.option,choice===value&&s.selected]}><Text style={[s.optionText,choice===value&&s.selectedText]}>{i18n.t(value.toLowerCase())}</Text></Pressable>)}</View>
