...
export default function Parcel(){
  ...
  return <SafeAreaView style={[s.safe,dark&&s.dark]}><ScrollView contentContainerStyle={s.content}>
    ...
    <Text style={s.kicker}>{i18n.t("orderNo")}</Text><Text style={[s.title,dark&&s.white]}>{parcel.orderId?.trim()||"—"}</Text>
    <Text style={s.muted}>{i18n.t("tracking")}: {parcel.trackingNumber}</Text>
    <Text style={s.muted}>{parcel.customerName} · {parcel.customerPhone??"—"}</Text><Pressable accessibilityRole="button" accessibilityLabel={i18n.t("callCustomer",{name:parcel.customerName})} onPress={()=>void call()} style={s.call}><Text style={s.callText}>☎ {i18n.t("call")}</Text></Pressable><Text style={s.muted}>{parcel.address}</Text><Text style={s.muted}>{i18n.t("parcelMoneyLine",{cod:parcel.codAmount.toLocaleString(),fee:parcel.deliveryFee.toLocaleString()})}</Text>
