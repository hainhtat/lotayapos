...
- Status override that leaves a money-bearing status (`DELIVERED` or `PARTIAL`) while **unreversed** money journals exist is rejected with `409 MONEY_POSTED`; there is no automatic reversal—Finance must reverse posted entries before the status correction. The gate considers parcel-scoped sources (`RIDER_COMMISSION`, `RIDER_RECEIVABLE_RECOGNITION`, `PARTIAL_RETURN_COLLECTION`, `OS_PARTIAL_RETURN_ADJUSTMENT`, `DELIVERY_COLLECTION`, including versioned sourceIds `parcelId:uuid`) and, when the parcel is linked, group-scoped `LINKED_DELIVERY_COLLECTION` / `LINKED_RIDER_COMMISSION` / `LINKED_RIDER_RECEIVABLE_RECOGNITION` / `LINKED_RIDER_RECEIVABLE_COD` / `LINKED_RIDER_RECEIVABLE_FEE` / `LINKED_OS_SHORTFALL` journals. Journals that already have a matching `LEDGER_REVERSAL` (reversal `sourceId` = original entry id) do not block the override. After reversal, re-post of money events uses free versioned journal `sourceId`s via `nextVersionedJournalSourceId` (`baseId` or `baseId:uuid`) for `RIDER_COMMISSION`, `RIDER_RECEIVABLE_RECOGNITION`, `BATCH_PICKUP_ADVANCE` (batch id), and the linked `LINKED_*` sources above so the unique source key can be reused.
...
- When recording a delivery outcome (`DELIVERED` / `PARTIAL` / `FAILED` / `REJECTED`) for an assigned rider, the server prefers completing an open `DeliveryWay`; if none exists (legacy data, ERP override that skipped `OUT_FOR_DELIVERY`, rider mismatch, etc.), it creates a completed way so ERP status transitions such as `OUT_FOR_DELIVERY → DELIVERED` succeed without a prior rider-started way.
...
- `DeliveryWay` with parcel, rider, assigned time, outcome, commission rate/base/amount, and completion time. Outcome recording may create a completed way when none is open so ERP can finish delivery without a prior rider-started way.
...
| Parcel guards | Status override leaving `DELIVERED`/`PARTIAL` with **unreversed** money journals (parcel-scoped + `LINKED_*` group journals when linked; ignores `LEDGER_REVERSAL`) → `409 MONEY_POSTED` (no auto-reversal); after reversal, re-post uses versioned `sourceId` (`baseId` or `baseId:uuid`) for `RIDER_COMMISSION`, `RIDER_RECEIVABLE_RECOGNITION`, `BATCH_PICKUP_ADVANCE`, and linked receivable/commission sources; COD/township edit after unreversed `BATCH_PICKUP_ADVANCE` → `409 ADVANCE_POSTED` |
...
4. Rider records Delivered, Partial Return, Failed, or Rejected (or Superadmin / Operations Manager / Dispatcher sets any allowed status from the ERP **Dispatch Queue**, including corrections broader than rider-allowed transitions); exception outcomes require reason codes, and Partial Return additionally requires Actual COD Collected. ERP outcome recording completes or creates a `DeliveryWay` when needed. Reschedule reason codes raise `DATE_CHANGE` alerts.
