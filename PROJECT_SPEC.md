...
5. **Implementation note:** backend supports Ops `PENDING_RETURN` / `RETURNED` transitions, pending-return extension, `GET /finance/os-pending-returns` (recoverable-only list fields), `POST /finance/os-returns/receive` (required `idempotencyKey`, staged OFFSET receive, replay **200**, `DEDUCTION_INCOMPLETE` / versioned re-post), and `POST /ledger/return-deductions`. ERP UI: All Batches (overview + alerts), Dispatch Queue (dispatch/assignment/status/Partial Return), and a dedicated Ops return queue; Finance OS & riders hosts the pending-return panel with Received (Auditor read-only) plus batch selection, statement preview, and post with COD-coverage gating and `returnedAdvanceContribution`-based `returnedAdvance` / OFFSET-clearing settlement journals (§6.6). Saved editable settlement drafts and post-post auditable settlement amendments are implemented.
...
- `backend/`: Express + Prisma API with JWT auth, English/Myanmar errors, validation, and targeted Jest/Supertest coverage. Implemented backend behavior includes phone/email/username login, server-paginated and filtered `listBatches`, atomic `POST /parcels/bulk-status`, parcel field history/audit, a dedicated Ops return queue, settlement drafts, auditable post-post settlement amendments, detailed profit/report endpoints, and idempotent return-deduction re-post after reversal. The current backend slice also covers the ledger, reconciliation, collection, shortfall, return, cashbook, reopen, and salary paths. Remaining work is external/manual: production PostgreSQL deploy/restore verification, real-time alert transport, observability, backups, and broader integration coverage.
