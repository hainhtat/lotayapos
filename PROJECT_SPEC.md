...
- Outstanding-rider reporting must include the number of unsettled riders and each rider’s outstanding amount for **rider cash collections**, plus the manual split-wallet payment history used to reach that balance. Dashboard/Overview should also show unsettled OS batches, the total **Outstanding to OS** in MMK, available OS credit, COD collected today, delivery fees, wallet balances, returns due, overdue returns, failed/partial delivery alerts, expense total, and net profit breakdown, with each card deep-linking to the filtered operational or Finance view that explains the total. Outstanding to OS is derived from the OS COD payable account balance and links to the OS & riders account workspace. The overview must distinguish paid-by-OS credit and physical-return credit from rider-cash receivables, so finance users can see the next advance reduction and the cash collection still requiring manual entry.
...
- Dashboard: today’s parcels, COD/fee totals, rider cash-collection outstanding settlements, unsettled OS batches, **Outstanding to OS** in MMK, available OS credit and next-advance reduction, wallet balances, returns due, overdue returns, alerts, expenses, and gross profit with visible profit components (or clear links into the profit report). The Outstanding to OS card is calculated from OS COD payable and links to the OS & riders workspace. Active-batches “view all” navigates to **All Batches** (`/operations/batches`); clicking a dashboard batch card opens **Dispatch Queue** filtered by that batch (`/operations/dispatch?batchId=…`). Each overview card should deep-link to the filtered Finance or Operations page that explains the total. Finance-facing overview cards must separate explicit paid-by-OS/physical-return credit from standard rider-cash receivables.
- Operations (split UI):
