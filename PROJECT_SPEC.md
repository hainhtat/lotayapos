...
- Treat explicitly paid-to-OS `DELIVERED` as an OS-credit event, not a rider cash obligation. Standard rider cash collections create rider receivable/outstanding and require a separate evening rider settlement.
...
- Only `DELIVERED` parcels create rider commission (when the pay model includes percentage commission). The explicit paid-to-OS action creates OS credit and no rider COD debt; standard rider cash collection creates the corresponding rider receivable/outstanding. `REJECTED`, `FAILED`, and `RETURNED` earn zero commission unless a future policy explicitly creates another payable status. Fixed salary components are independent of individual way outcomes.
...
- Legacy parcel records retain their per-parcel advance values. For a newly created automatic-accounting batch, the pickup advance is entered once at batch creation as an optional Cash/KBZ Pay/Wave Pay split. The saved batch and its wallet movement are one atomic, idempotent action; the entered advance may be less than, equal to, or (with explicit authorization) greater than the batch COD as parcels are added. Negative OS positions remain credits/receivables and are never silently netted away.
...
- Dispatch Queue provides an explicit **Delivered — paid to OS** action in addition to the standard rider/cash delivered workflow. It displays the resulting OS credit before confirmation: parcel COD plus the full configured delivery fee when the OS agreement includes it. This creates no rider COD debt and no wallet receipt. Available OS credit automatically reduces the requested advance for the next batch in the same OS/hub and carries any unused amount forward. Standard rider cash collection creates the rider-side collection obligation; actual remittance is recorded later through a manual rider settlement that may split the declared amount across Cash, KBZ Pay, and Wave Pay, and the remaining cash-collection outstanding balance stays visible until that settlement is posted. Unfulfilled parcel outcomes such as `FAILED`, `RETURNED`, `REJECTED`, `PARTIAL`, and `PENDING_RETURN` remain separate operational statuses and do not by themselves imply payment receipt or payment settlement.
...
- New batches use automatic accounting: create the batch with the optional advance wallet split, create/update its OS COD obligation as parcels are saved, and allow operations to continue adding parcels after finalize. The older explicit pickup-advance endpoint remains only for non-automatic legacy batches.
...
For a new automatic-accounting batch, a Superadmin, Finance user, or Operations Manager may save the configured pickup advance against `OS_COD_PAYABLE` and the selected company wallets in the same transaction. An OS batch COD obligation is created immediately and is synchronized as parcels are saved or their COD changes; finalizing the batch does not prevent later parcel entry for this workflow. The operational OS account is a per-OS, per-hub list of these batch obligations, not a new editable settlement statement. The older explicit pickup-advance endpoint remains available for legacy non-automatic batches.

For each obligated batch, the displayed balance is calculated as:

`outstanding = max(0, original COD + approved opening adjustment − posted advance − applied OS credits − posted OS-payment allocations)`.

An OS credit is created either by an explicit paid-to-OS delivery or by confirmed physical return to OS. A paid-to-OS delivery credits its COD and includes the full configured delivery fee only where that fee is marked as included by the OS. Physical-return credit is COD-only; a returned parcel's delivery fee is not an OS credit or a return deduction unless its persisted paid-to-OS fee-included decision says the OS had already paid that fee. The same fee rule is used in legacy OS-settlement components and return-handover totals. If the expression is negative, the excess remains available OS credit rather than a negative payable. Available credit is automatically applied to reduce the next batch's advance requirement for the same OS and hub, oldest-first, with any surplus carried forward. Credit consumption and cash payments allocate oldest-first across selected outstanding batches, are hub-scoped, and are recorded atomically with idempotency protection. Payments may be split across Cash, KBZ Pay, and Wave Pay. A wallet payment cannot exceed the selected payable after available credit.
...
- Paid-to-OS delivery: create OS credit for COD plus the full configured delivery fee only when that fee is included by the OS; do not create rider COD receivable. Recognize delivery-fee revenue and commission according to policy.
- Rider cash collection (technical mode `CASH_RECEIPT_EXCEPTION`): debit selected rider-clearing/settlement account; credit `Customer COD Receivable`; separately recognize fee revenue and commission according to policy. This is the only delivery mode that creates rider COD receivable.
- Rider remittance: debit wallet(s); credit `Rider Settlement Receivable` (`RIDER_RECEIVABLE`) for rider cash collections. When a daily salary deduction applies, post a visible `RIDER_RECEIVABLE` credit for remittance before salary and a `RIDER_RECEIVABLE` debit for `salaryDeduction` in the same settlement journal (description includes the salary amount); no new account code.
...
1. Superadmin, Finance, or Operations Manager creates an automatic-accounting batch for one OS and exact pickup date, entering any pickup advance as a Cash/KBZ Pay/Wave Pay split. Dispatcher may create a zero-advance batch.
2. One idempotent save creates the batch, its OS obligation, and—when an advance was entered—the balanced `BATCH_PICKUP_ADVANCE` wallet movement. A browser refresh can retry the same saved request without duplicating the batch or wallet movement.
3. Operations adds/imports parcels. The batch COD obligation is synchronized to the saved parcels; finalization is an operational marker and does not close automatic batches to subsequent parcel entry.
4. Batch inventory and the OS payable update immediately. The legacy Finance post-pickup-advance action applies only to older non-automatic batches.
...
2. Operations monitors and resolves overdue returns via **Dispatch Queue** parcel status (`/operations/dispatch`) and/or a dedicated Ops return-queue surface when present; overdue Pending Return alerts surface on **All Batches**. The direct Pending Return action requires a free-text reason recorded in status history. The Return to OS queue can prepare a branded landscape PDF handover preview/download with wrapped customer, address, and reason/note fields, without changing parcels or finance. Physical handoff is confirmed separately; only that confirmation changes an eligible parcel to `RETURNED` and creates OS credit. Timer extensions remain audited as already specified.
...
6. Explicit paid-to-OS `DELIVERED` has no rider COD obligation. Standard rider cash collection creates rider outstanding, which is cleared when its evening settlement is manually recorded and verified.
...
- Failed parcels require an explicit retry-tomorrow, reschedule-date, or Return to OS decision; cancellations are recorded as `REJECTED` and enter Return to OS with the configured reason/note. A direct Pending Return status requires one free-text reason and records it in status history. Return-to-OS handover PDF preview/download does not change parcel or finance state; it uses a landscape multi-line layout for customer, address, and reason/note, and excludes return delivery fees unless the OS-included fee decision was persisted. Operations can resolve Pending Returns via Operations return queue / parcel status (`PENDING_RETURN → RETURNED`). Finance can confirm physical return-to-OS via **Received** on the OS & riders pending-return panel (`FAILED` / `REJECTED` / `PENDING_RETURN` / `PARTIAL` → `RETURNED`), creating a batch return credit without a wallet credit. The OS account applies that credit and split-wallet OS payments to batch COD payables; payment corrections are non-destructive and legacy settlement history is retained read-only in the collapsed archived section after cutover.
- A new automatic-accounting batch can save its advance wallet split and later parcel COD in one continuous workflow: Superadmin, Finance, or Operations Manager can record the advance, it is posted exactly once on batch creation, the OS obligation tracks subsequent parcel COD changes, and retrying the same creation request does not duplicate money movement. Legacy batches retain their explicit posting workflow.
...
- An explicit paid-by-OS `DELIVERED` parcel creates OS credit for COD plus the full configured delivery fee only when the OS includes that fee, and creates no rider COD debt. Paid-by-OS and confirmed physical-return credits automatically reduce the next batch advance for the same OS/hub, with surplus carried forward. Standard rider cash collection is the delivery mode that creates rider receivable requiring manual settlement.
