...
2. Operations monitors and resolves overdue returns via **Dispatch Queue** parcel status (`/operations/dispatch`) and/or a dedicated Ops return-queue surface when present; overdue Pending Return alerts surface on **All Batches**. The Return to OS queue can prepare a branded PDF handover preview/download without changing parcels or finance. Physical handoff is confirmed separately; only that confirmation changes an eligible parcel to `RETURNED` and creates OS credit. Timer extensions remain audited as already specified.
