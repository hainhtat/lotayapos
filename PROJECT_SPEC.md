...
For a new automatic-accounting batch, saving the batch records the configured pickup advance against `OS_COD_PAYABLE` and the selected company wallets in the same transaction. An OS batch COD obligation is created immediately and is synchronized as parcels are saved or their COD changes; finalizing the batch does not prevent later parcel entry for this workflow. The operational OS account is a per-OS, per-hub list of these batch obligations, not a new editable settlement statement. The older explicit pickup-advance endpoint remains available for legacy non-automatic batches.
...
- Outstanding-rider reporting must include the number of unsettled riders and each rider’s outstanding amount, plus the manual split-wallet payment history used to reach that balance. Dashboard/Overview should also show unsettled OS batches, the total **Outstanding to OS** in MMK, COD collected today, delivery fees, wallet balances, returns due, overdue returns, failed/partial delivery alerts, expense total, and net profit breakdown, with each card deep-linking to the filtered operational or Finance view that explains the total. Outstanding to OS is derived from the OS COD payable account balance and links to the OS & riders account workspace. The overview should also make room for a quick status of rider receivables and settlement completion so finance users can see what still needs manual entry.
...
- `Batch` stores automatic-accounting/idempotency metadata for new pickup-and-advance records and optional one-to-one historical OS settlement evidence. `Parcel` stores an optional planned delivery date for rescheduling; its original created timestamp remains the age source for operations queues.
...
| Batches/parcels | `GET/POST /operations/batches` (new batch request may contain `advancePaid`, split `wallets`, and idempotency key), `POST /batches/import`, `GET/PATCH /parcels`, `POST /parcels/reschedule`, `GET /parcels/:id/history` |
...
- Finance: two tabs—**Finance overview** (default) and **OS & riders** (`?tab=settlements`). Deep links open the settlements tab. New batch creation is the primary pickup-advance entry; the Finance overview retains the legacy post-pickup advances panel only for older non-automatic batches, alongside wallet/ledger balances and related finance surfaces. Its wallet cards expose a Superadmin-only **Adjust wallet balance** dialog: select the displayed wallet, hub, business date, actual balance in hand, and a reason; the calculated difference is posted as an auditable adjustment rather than changing history. OS & riders hosts OS account balances by shop and batch, split-wallet payment/credit application, payment history and correction, the pending-return Received action (Auditor read-only), read-only legacy settlement history, and rider outstanding balances, plus cashbook, reconciliation, day-close, ledger drill-down, and profit visibility.
...
1. Finance or Superadmin creates an automatic-accounting batch for one OS and exact pickup date, entering any pickup advance as a Cash/KBZ Pay/Wave Pay split. Operations/Dispatcher may create a zero-advance batch.
...
| Create batch with advance / settle OS | Full | Create zero-advance batch | Full | Create zero-advance batch | No | View |
