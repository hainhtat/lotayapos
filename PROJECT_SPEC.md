...
4. Rider records Delivered, Partial Return, Failed, or Rejected (or Superadmin / Operations Manager / Dispatcher sets any allowed status from the ERP **Dispatch Queue**, including corrections broader than rider-allowed transitions). A normal Delivered outcome is paid by OS and records OS credit; a cash receipt must be explicitly marked as the exception. Exception outcomes require reason codes, and Partial Return additionally requires Actual COD Collected. ERP `OUT_FOR_DELIVERY` / outcome recording keeps one primary `DeliveryWay` (prefer current rider, else oldest open), closes other open ways with outcome `SUPERSEDED`, and creates a completed way when none is open. Reschedule reason codes raise `DATE_CHANGE` alerts.
5. ERP updates inventory, OS credit or cash-exception receivable, commission eligibility, and return queue.

### Pending return and OS recovery

1. Partial/Failed/Rejected enters `PENDING_RETURN` with due date = entry date + four hub-timezone calendar days (Partial may remain `PARTIAL` until finance/ops resolution; finance recovery queue also includes `PARTIAL` / `FAILED` / `REJECTED` before or without that intermediate status). A date change is a separate reschedule action: it stores a planned delivery date/reason, leaves the parcel in its original batch, and does not reset the original age.
...
- A reschedule stores its intended next delivery date/reason without moving the parcel out of its original batch or resetting its age. Dispatch Queue provides focused To assign, With riders, Rescheduled, Return to OS, and unresolved 3+ days-in-hand queues; the latter is calculated from original parcel creation time.
